...
static void serialTimerRxConfig(const timerHardware_t *timerHardwarePtr, uint8_t reference, portOptions_t options)
{
    ...
    serialICConfig(timerHardwarePtr->tim, timerHardwarePtr->channel,
            (options & SERIAL_INVERTED) ? TIM_ICPolarity_Rising : TIM_ICPolarity_Falling);
...
void prepareForNextRxByte(softSerial_t *softSerial)
{
    ...
    if (softSerial->rxEdge == LEADING) {
        ...
        serialICConfig(softSerial->rxTimerHardware->tim, softSerial->rxTimerHardware->channel,
...
void onSerialRxPinChange(timerCCHandlerRec_t *cbRec, captureCompare_t capture)
{
    ...
    if (softSerial->isSearchingForStartBit) {
        ...
        serialICConfig(softSerial->rxTimerHardware->tim, softSerial->rxTimerHardware->channel,
                inverted ? TIM_ICPolarity_Falling : TIM_ICPolarity_Rising);
    ...
    if (softSerial->rxEdge == TRAILING) {
        ...
        softSerial->rxEdge = TRAILING;
        serialICConfig(softSerial->rxTimerHardware->tim, softSerial->rxTimerHardware->channel,
                inverted ? TIM_ICPolarity_Rising : TIM_ICPolarity_Falling);
    }
}
...
const struct serialPortVTable softSerialVTable[] = { { softSerialWriteByte, softSerialRxBytesWaiting, softSerialTxBytesFree, softSerialReadByte, softSerialSetBaudRate, isSoftSerialTransmitBufferEmpty, softSerialSetMode, } };

...
#define CMD_WHO_AM_I			0x0F // Who am I?
#define CTRL_REG1				0x10 //
#define CTRL_REG2				0x11 //
#define CTRL_REG3				0x12 //
#define LPS22HB_WHO_AM_I		0xB1 //Response of who am I
...
bool lps22hbDetect(baro_t *baro)
{
    ...
    ack = i2cRead(LPS22HB_ADDR, CMD_WHO_AM_I, 1, &sig);
    if (ack && (sig == LPS22HB_WHO_AM_I)) {
...
{
    i2cWrite(LPS22HB_ADDR, CMD_RESET, 1);
...
{
    uint8_t rxbuf[2] = { 0, 0 };
    i2cRead(LPS22HB_ADDR, CMD_PROM_RD + coef_num * 2, 2, rxbuf); // send PROM READ command
...
{
    uint8_t rxbuf[3];
    i2cRead(LPS22HB_ADDR, CMD_ADC_READ, 3, rxbuf); // read ADC
    return (rxbuf[0] << 16) | (rxbuf[1] << 8) | rxbuf[2];
}

static void ms5611_start_ut(void)
{
    i2cWrite(LPS22HB_ADDR, CMD_ADC_CONV + CMD_ADC_D2 + ms5611_osr, 1); // D2 (temperature) conversion start!
}
...
static void ms5611_start_up(void)
{
    i2cWrite(LPS22HB_ADDR, CMD_ADC_CONV + CMD_ADC_D1 + ms5611_osr, 1); // D1 (pressure) conversion start!
}

...
void processRcStickPositions(rxConfig_t *rxConfig, throttleStatus_e throttleStatus, bool retarded_arm, bool disarm_kill_switch)
{
    ...
    if (!isUsingSticksToArm) {
        ...
        if (IS_RC_MODE_ACTIVE(BOXARM)) {
            ...
            } else if (netAccMagnitude < 11 && (!status_FSI(Crash))) {//&&inclination.values.pitchDeciDegrees <=100&&inclination.values.pitchDeciDegrees >=-100&&inclination.values.rollDeciDegrees >= -100 && inclination.values.rollDeciDegrees <= 100){//Chuck to ARM Check for //intern Akash

...
const char *NMEAmtk="$PMTK251,115200*1F\r\n";

uint32_t buadChangeLoop=0;
bool startbuadchangeloop=false;
    ...
    switch (gpsData.state) {
        case GPS_INITIALIZING:
            ...
            if (gpsData.state_position < 1) {
                ...
                serialPrint(gpsPort, "$PMTK251,115200*1F\r\n");
               // LED_M_ON;
                gpsData.state_position++;
            } else {
        ...
        case GPS_CHANGE_BAUD:
        	  ...
        	  now = millis();
        	           ...
        	           gpsData.state_ts = now;
        	           if (gpsData.state_position < 1) {
        	              serialSetBaudRate(gpsPort,115200);
    ...
    switch (gpsData.state) {
        case GPS_INITIALIZING:
            ...
            if (gpsData.state_position < GPS_INIT_ENTRIES) {
                ...
                baudRate_e newBaudRateIndex =(baudRate_e) gpsInitData[gpsData.state_position].baudrateIndex;
        ...
        case GPS_CONFIGURE:
            ...
            if (gpsData.messageState == GPS_MESSAGE_STATE_IDLE) {
                gpsData.messageState=(gpsMessageState_e)(gpsData.messageState+1);
            }
...
void gpsEnablePassthrough(serialPort_t *gpsPassthroughPort)
{
    ...
    if (!(gpsPort->mode & MODE_TX))
        serialSetMode(gpsPort,(portMode_t)( gpsPort->mode | MODE_TX));

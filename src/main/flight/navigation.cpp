...
int32_t posRateIntegralLat=0;

int32_t posRateIntegralLon=0;
...
#define POSHOLD_RATE_IMAX      10       // degrees
...
#define GPS_FILTERING              0    // add a 5 element moving average filter to GPS coordinates, helps eliminate gps noise but adds latency
...
void onGpsNewData(void)
{
    ...
    if (!STATE(GPS_FIX_HOME)&& ARMING_FLAG(ARMED))  // && ARMING_FLAG(ARMED)
...
void gpsUsePIDs(pidProfile_t *pidProfile)
{
    ...
    poshold_ratePID_PARAM.Imax = POSHOLD_RATE_IMAX * 10;
...
static void GPS_calc_location_error(int32_t *target_lat, int32_t *target_lng, int32_t *gps_lat, int32_t *gps_lng)
{
    //   error[LON] = (float) (*target_lng - *gps_lng) * GPS_scaleLonDown;   // X Error
	 error[LON] = *target_lng - *gps_lng;

	 lonError = (error[LON]*111)/100;
	 //lonError= sqrtf(sq((float)(*target_lng - *gps_lng))) * DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR_IN_HUNDREDS_OF_KILOMETERS;

	error[LAT] = *target_lat - *gps_lat;        // Y Error

	 latError = (error[LAT]*111)/100;
	 //latError= sqrtf(sq((float)(*target_lat - *gps_lat))) * DISTANCE_BETWEEN_TWO_LONGITUDE_POINTS_AT_EQUATOR_IN_HUNDREDS_OF_KILOMETERS;
...
static void GPS_calc_poshold(void)
{
    ...
    for (axis = 0; axis < 2; axis++) {
        ...
        if(!ARMING_FLAG(ARMED))
        poshold_ratePID[axis].integrator = 0;

        if(axis==LAT)
        { posRateIntegralLat=get_I(rate_error[axis] + error[axis], &dTnav, &poshold_ratePID[axis], &poshold_ratePID_PARAM);

        nav[axis] = get_P(rate_error[axis], &poshold_ratePID_PARAM) +posRateIntegralLat;
        }
        else
        {
        	posRateIntegralLon=get_I(rate_error[axis] + error[axis], &dTnav, &poshold_ratePID[axis], &poshold_ratePID_PARAM);

        	        nav[axis] = get_P(rate_error[axis], &poshold_ratePID_PARAM) +posRateIntegralLon;
        ...
        if (IS_RC_MODE_ACTIVE(BOXGPSHOME)) {
            ...
            if (IS_RC_MODE_ACTIVE(BOXGPSHOLD) && areSticksInApModePosition(gpsProfile->ap_mode)) {
            	 ...
            	 if (!FLIGHT_MODE(GPS_HOLD_MODE)) {
                    ...
                    GPS_hold[LAT] =192326790;
                    GPS_hold[LON] = 729157020;
